...
app.get("/accounts", (req, res) => {
  if (req.query.website_key != undefined) {
    accounts
      .getUserViaWebsiteKey(req.query.website_key)
      .then((response) => {
    ...
    accounts
      .getUserViaDiscordId(req.query.discord_id)
      .then((response) => {
        res.status(200).send(response);
      })
      .catch((error) => {
...
app.post("/accounts", (req, res) => {
  // receives axios post requests to input account data into the db
  if (
...
app.get("/timeline", (req, res) => {
  if (req.query.website_key != undefined) {
    timeline
      .getTimelineAssignmentObjectsViaWebsiteKey(req.query.website_key)
      .then((response) => {
        res.status(200).send(response);
      })
      .catch((error) => {
    ...
    timeline
      .getTimelineViaGuildId(req.query.guild_id)
      .then((response) => {
        res.status(200).send(response);
      })
      .catch((error) => {
    ...
    req.query.timeline_id != undefined &&
    req.query.ALL_OBJECTS != undefined
  ) {
    timeline
      .getAllTimelineObjectsWithTimelineId(req.query.timeline_id)
      .then((response) => {
        res.status(200).send(response);
      })
      .catch((error) => {
...
app.post("/timeline", (req, res) => {
  if (req.body.DELETE === undefined) {
    ...
    if (
      ...
      timeline
        .deleteTimelinePermission(req.body.timeline_id, req.body.discord_id)
        .then((response) => {
          res.status(200).send(response);
        })
        .catch((error) => {
      ...
      timeline
        .deleteTimeline(req.body.timeline_id)
        .then((response) => {
          res.status(200).send(response);
        })
        .catch((error) => {
